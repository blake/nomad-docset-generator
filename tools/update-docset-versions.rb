...
built_docsets.each do |docset|
  ...
  # This version already exists in the docset list. Do a further comparsion
  # by checking if the sha1sum of the file in the archive matches the
  # sha1sum of the file on disk. If not, add the new file to the docset
  # list.

  archive_sub_version_string = "#{format('%03d', latest_sub_version)}-#{file_sha1shortsum}"
  archive_file = "#{docset[:version]}/Nomad-#{archive_sub_version_string}.tgz"
  new_version = {
